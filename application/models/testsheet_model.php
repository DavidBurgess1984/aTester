<?php 
class Testsheet_model extends CI_Model {
    ...
    function isCustomTestsheet($testsheetID){
        ...
        return (intval($result->tsIdTestSheetTemplate) === 255) ? true : false;
    }
    
    function hasNoCategories($testsheetID){
        
        $sql = ' SELECT COUNT(*) AS \'count\' FROM  Category WHERE cIdTestsheet = %1$d AND cIdTestsheetTemplate = 255';
    ...
    function loadTestsheetsByProject($projectID){
    	$sql = 'SELECT 
                    idTestSheet as `id`,
                    tsTitle as `title`,
                    tsIdTestSheetTemplate as \'custom_type\'
                FROM
                    TestSheet
                WHERE 
                    tsIdProject = %1$s';
